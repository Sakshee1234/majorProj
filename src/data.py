...
class BaseDataManager():
    ...
    def numericalize(self, token_list):
        ...
        return torch.tensor([self.vocab.word2int.get(token, self.vocab.word2int['<unk>']) for token in token_list])
...
class SingleTweetDataManager(BaseDataManager):
    ...
    def custom_collate(self, batch):
        ...
        labels = torch.tensor([int(example['label']) for example in batch], device=self.device) #(5)
